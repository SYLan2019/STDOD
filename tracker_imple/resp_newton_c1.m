function [disp_row, disp_col, sind, max_scale_response, response,w] = resp_newton_c1(response1, response2, iterations, ky, kx, use_sz)
%% fuse response and find the maxii???
...
if numel(response2) ~= 1 && numel(response1) ~= 1
    ...
    r1 = response1(:,:,i);
    %r2 = response2(:,:,i);
    r2 = response2;% only use one deep feature???
...
%    tmpfuserespsz = size(response)% 7scales
%    szresp2tmp =size(response2)% 1scale
...
max_response = 1 / prod(use_sz) * real(mtimesx(mtimesx(exp_iky, responsef, 'speed'), exp_ikx, 'speed'));%size 1*1*7?
...
%%%%%%%%%add 2019-8-16--want to only use hand feature to update scale--<
[max_resp_row1, max_row1] = max(response1, [], 1);
[init_max_response1, max_col1] = max(max_resp_row, [], 2);
max_row_perm1 = permute(max_row1, [2 3 1]);
col1 = max_col1(:)';
row1 = max_row_perm(sub2ind(size(max_row_perm1), col1, 1:size(response,3)));

%max_response = 1 / prod(use_sz) * real(mtimesx(mtimesx(exp_iky, responsef1, 'speed'), exp_ikx, 'speed'));%only use hand feature?
[max_scale_response1, sind1] = max(response1(:));%%%%%%%%%%%%%%
%----------------------->
