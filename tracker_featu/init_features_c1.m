function [features, gparams, feature_info] = init_features_c1(features, gparams, params, is_color_image, img_sample_sz, size_mode)
...
for k = 1:length(features)
   ...
   elseif isequal(features{k}.getFeature, @get_dagnn_layers_c1) 
        if ~isfield(features{k}.fparams, 'input_size_mode')
            features{k}.fparams.input_size_mode = 'adaptive';
        end
        if ~isfield(features{k}.fparams, 'input_size_scale')
...
if simplenn_feature_ind > 0 %here only dispose simplenn deep feature.
    ...
    fprintf('fortest simplenn_feature_ind > 0 set support img sz here===%d,%d\n',feature_info.img_support_sz{simplenn_feature_ind});%not here
    % Set the input size
...
if dagnn_feature_ind > 0 %here only dispose dagnn deep feature.
    ...
    %fprintf('***for test here dispose the imgsamplesz of dagnn deep feature ==%d,%d\n',new_img_sample_sz);
    ...
    %fprintf('__________fortest simplenn_feature_ind >, 0 set support img sz here===%d,%d\n',feature_info.img_support_sz{dagnn_feature_ind});%240???
...
for k = 1:length(features)
    if k ~= dagnn_feature_ind && k ~= simplenn_feature_ind %here only dispose not cnn feature
        orig_sz = round(img_sample_sz(1,:)/feature_info.min_cell_size(k));
        if strcmpi(size_mode, 'odd_cells')
            desired_sz = orig_sz + mod(orig_sz,2) +1;%51,35
            temp_size = round(img_sample_sz(1,:));%200
            %tempcellsz(k) = feature_info.min_cell_size(k)%4,6
        ...
        %ftype(k) =features{k}.is_cnn%for test .all these k(==1,2) ,their type not cnn?????
        %fprintf('___for test_______feature_info.img_sample_sz{k=%d}===%d,%d\n',k,feature_info.img_sample_sz{k});
    end
end
...
for k = 1:length(features)
    ...
    elseif strcmp(features{k}.is_cnn,'dagnn')
        features{k}.img_sample_sz = feature_info.img_sample_sz{k};
        %tmp_ft_imsamplesz = features{k}.img_sample_sz%for test ...
        %fprintf('___for test_______dagnn feature--features{k=%d}.img_sample_sz===%d,%d\n',k,features{k}.img_sample_sz);
